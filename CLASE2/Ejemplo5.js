...
const componenteListaDeAlquileres = rented.reduce((acumulador, item) => {
    return `${acumulador}
    *${item.id}-${item.title}`
}, ``)

const componenteListaDeAsociados = partners.reduce((acumulador, item)=>{
    return `${acumulador}
    *${item}`
}, ``)
...
let lineas = `
    ...
    Alquileres: 
        ${componenteListaDeAlquileres}
    Habilitado:${objeto.enabled ? "SI" : "NO"}
    Saldo : ${banlance < 0 ? `-${banlance}` : (banlance > 0 ?  `+${banlance}` : "Estas al dia")}
    Asociados : ${ partners.length === 0 ? "No tienes asociados" : componenteListaDeAsociados}`
console.log(lineas)